...
def initialize_metrics_csv():
    ...
    fieldnames = ["timestamp", "Latency", "Throughput", "CheckpointID", "CheckpointStatus", "CheckpointDurationMs", "JobStatus"]
...
def write_metrics_to_csv(timestamp, latency, throughput, checkpoint_id=None, checkpoint_status=None, checkpoint_duration_ms=None, job_status=None):
    """将指标与 checkpoint 信息写入 metrics.csv 文件。"""
    fieldnames = ["timestamp", "Latency", "Throughput", "CheckpointID", "CheckpointStatus", "CheckpointDurationMs", "JobStatus"]
    data = {
        "timestamp": timestamp,
        "Latency": latency if latency is not None else "N/A",
        "Throughput": throughput if throughput is not None else "N/A",
        "CheckpointID": checkpoint_id if checkpoint_id is not None else "N/A",
        "CheckpointStatus": checkpoint_status if checkpoint_status is not None else "N/A",
        "CheckpointDurationMs": checkpoint_duration_ms if checkpoint_duration_ms is not None else "N/A",
        "JobStatus": job_status if job_status is not None else "N/A"
    }
    try:
...
def get_checkpoint_info():
    """
    获取作业的 checkpoint 信息（尝试解析最近一次完成的 checkpoint），
    返回包含 id、status、以及 end-to-end duration（毫秒）的字典，或 None 如果不可用。
    """
    if not JOB_ID:
        return None
    try:
        url = f"{BASE_URL}/jobs/{JOB_ID}/checkpoints"
        ...
        if not entry:
            ...
            if completed:
                if isinstance(completed, dict) and 'id' in completed:
                    entry = completed
                else:
                    c_entries = completed.get('entries') if isinstance(completed, dict) else None
                    if c_entries:
                        entry = c_entries[0]

        if not entry:
            entry = data.get('last') or data.get('last_completed')

        if not entry:
            return None

        duration = None
        for key in ['end_to_end_duration', 'end-to-end-duration', 'endToEndDuration', 'duration', 'checkpoint_duration', 'end_to_end_latency']:
            if key in entry:
                duration = entry.get(key)
                break

        checkpoint_id = entry.get('id') or entry.get('checkpoint_id') or entry.get('checkpointId')
        status = entry.get('status') or entry.get('state')

        if duration is None:
            trigger = entry.get('trigger_timestamp') or entry.get('triggerTimestamp') or entry.get('trigger')
            complete = entry.get('completion_timestamp') or entry.get('completionTimestamp') or entry.get('completion') or entry.get('latest_ack_timestamp')
            try:
                if trigger and complete:
                    duration = int(complete) - int(trigger)
            except Exception:
                duration = None
...
def main():
    ...
    try:
        while True:
            ...
            # 获取 checkpoint 信息并合并写入 metrics.csv
            cp_id = None
            cp_status = None
            cp_duration = None
            cp_info = get_checkpoint_info()
            if cp_info:
                cp_id = cp_info.get('checkpoint_id')
                cp_status = cp_info.get('checkpoint_status')
                cp_duration = cp_info.get('checkpoint_duration_ms')
                print(f"🔁 [{current_time.strftime('%H:%M:%S')}] Checkpoint: id={cp_id}, status={cp_status}, duration_ms={cp_duration}")

            # 写入 metrics.csv (使用微秒精度时间戳)，包含 checkpoint 与 job 状态
            write_metrics_to_csv(
                ...
                checkpoint_duration_ms=cp_duration,
